...
router.post("/create", isAuthenticated, async (req, res, next) => {
  ...
  try {
    const { title, genre, image, description, location, date, creator } =
      req.body;
...
router.get("/artist/:id", async (req, res, next) => {
  try {
    ...
    const events = await Event.find({ artists: artist });
    res.status(200).json(events);
  } catch (error) {
    res.json(error);
    next(error);
...
router.put("/update/:id", async (req, res, next) => {
  try {
    const { title, genre, image, description, artists, location, date } =
      req.body;
...
//add artist to event route

router.put("/addartists/:id", async (req, res, next) => {
  try {
